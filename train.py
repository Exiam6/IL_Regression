...
def train(model, train_data_loader,val_data_loader, device, criterion, optimizer, args):
    ...
    for epoch in range(1, args.num_epochs + 1):
        metrics = train_epoch(model, epoch,train_data_loader, criterion, optimizer, device, args, accum_size=10 )
        metrics_valid = validate_epoch(model, epoch,val_data_loader, criterion, optimizer, device, args, accum_size=10 )
...
def calculate_metrics(metrics, device, y_dim):
    ...
    U = gram_schmidt(W)
    P_E = torch.mm(U.T, U)
    H_projected_E = torch.mm(H_norm, P_E)
    H_projected_E_norm = F.normalize(torch.tensor(H_projected_E).float().to(device), p=2, dim=1)
    result['projection_error_H2W_E'] = F.mse_loss(H_projected_E_norm, H_norm).item()
